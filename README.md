...
| 🎓 **Test** | A graded **assessment**, not a drill: pick **10 / 20 / 30** questions; **no hints, no show-answer, no auto-play, one shot per question**, and the whole thing is **timed** (total and per question). Question types are mixed and randomly interleaved — EN→Thai typed, Thai→EN typed, listening (tap 🔊, type the meaning), tone identification and full-sentence building — and words are sampled **weighted toward weak and older / long-unseen words**, not just the newest day. Matching is **stricter than Practice**: missing tone marks are still fine, but a near-miss that is (or is closer to) a *different* bank word is wrong, typed tone marks that spell a different word (*mǎi* for *mâi*) are wrong, and sentences need every word exact. The **results screen** shows score, time, accuracy per category and per question type, every wrong answer with the correct one, per-question times, a comparison with your previous test of the same length, and a button to send just the missed words into Practice (the **🎓 Test misses** filter). **History** is saved so you can see whether scores and times are improving. Use it sparingly — about weekly; practise daily in between. Answers do still feed the normal per-word scheduling. |
